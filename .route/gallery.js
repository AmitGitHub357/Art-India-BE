...
router.post("/", jwt.authenticateToken, upload.array("images"), function (req, res, next) {
    try {
        ...
        db.get()
            ...
            .insertOne(data, function (err, dbresult) {
                if (err)
                    res.status(500).send(httpUtil.error(500, "gallery Creation Failed."));
                res.send(httpUtil.success(200, "gallery Created."));
            });
    } catch (err) {
        res.send({
            status: 400,
...
router.delete("/", jwt.authenticateToken, function (req, res, next) {
    try {
        const gallery_id = req.query.gallery_id ? ObjectId(req.query.gallery_id) : "";
        if (gallery_id) {
            // unlinkAsync(req.file.path)
            db.get()
                .collection("gallery")
                .deleteOne({ _id: gallery_id }, function (err, result) {
                    if (err)
                        res.status(204).send(httpUtil.error(204, "gallery deletion error."));
                    res.send(httpUtil.success(200, "gallery deleted."));
                });
        } else {
            res.status(204).send(httpUtil.error(204, "gallery ID is missing."));
        }
    } catch (err) {
        res.send({
            status: 400,
